...
namespace Parse
{
    ...
    public class ParseObject : IEnumerable<KeyValuePair<string, object>>, INotifyPropertyChanged
    {
        ...
        {
            IEnumerable<object> items = DeepTraversalInternal(root, traverseParseObjects, new HashSet<object>(new IdentityEqualityComparer<object>()));
            if (yieldRoot)
        ...
        {
            seen.Add(root);
            System.Collections.IEnumerable itemsToVisit = isCompiledByIL2CPP ? null : (IEnumerable<object>) null;
            IDictionary<string, object> dict = Conversion.As<IDictionary<string, object>>(root);
            if (dict != null)
            {
            ...
            if (itemsToVisit != null)
            {
                foreach (object i in itemsToVisit)
                {
                    ...
                    {
                        yield return i;
                        IEnumerable<object> children = DeepTraversalInternal(i, traverseParseObjects, seen);
        ...
        private static Task DeepSaveAsync(object obj, string sessionToken, CancellationToken cancellationToken)
        {
            ...
            List<Task> saveDirtyFileTasks = DeepTraversal(obj, true).OfType<ParseFile>().Where(f => f.IsDirty).Select(f => f.SaveAsync(cancellationToken)).ToList();
            ...
            return Task.WhenAll(saveDirtyFileTasks).OnSuccess(_ =>
            {
                ...
                return InternalExtensions.WhileAsync(() => Task.FromResult(remaining.Any()), () =>
                {
                    ...
                    List<ParseObject> current = (from item in remaining where item.CanBeSerialized select item).ToList();
                    List<ParseObject> nextBatch = (from item in remaining where !item.CanBeSerialized select item).ToList();
                    remaining = nextBatch;
                    ...
                    return ParseObject.EnqueueForAll<object>(current, toAwait =>
                    {
                        return toAwait.OnSuccess(__ =>
                        {
                            ...
                            IList<Task<IObjectState>> saveTasks = ObjectController.SaveAllAsync(states,
                                operationsList,
                                sessionToken,
                                cancellationToken);
                            ...
                            return Task.WhenAll(saveTasks).ContinueWith(t =>
                            {
                                ...
                                {
                                    IObjectState[] serverStates = t.Result;
        ...
        public static Task SaveAllAsync<T>(
            IEnumerable<T> objects, CancellationToken cancellationToken) where T : ParseObject => DeepSaveAsync(objects.ToList(), ParseUser.CurrentSessionToken, cancellationToken);
        ...
        /// </summary>
        /// <param name="cancellationToken">The cancellation token.</param>
        internal Task<ParseObject> FetchAsyncInternal(CancellationToken cancellationToken) => taskQueue.Enqueue(toAwait => FetchAsyncInternal(toAwait, cancellationToken),
                cancellationToken);
        ...
        /// </summary>
        /// <param name="cancellationToken">The cancellation token.</param>
        internal Task<ParseObject> FetchIfNeededAsyncInternal(CancellationToken cancellationToken) => taskQueue.Enqueue(toAwait => FetchIfNeededAsyncInternal(toAwait, cancellationToken),
              cancellationToken);
        ...
        public static Task<IEnumerable<T>> FetchAllIfNeededAsync<T>(
            IEnumerable<T> objects) where T : ParseObject => FetchAllIfNeededAsync(objects, CancellationToken.None);
        ...
        public static Task<IEnumerable<T>> FetchAllAsync<T>(
            IEnumerable<T> objects) where T : ParseObject => FetchAllAsync(objects, CancellationToken.None);
        ...
        private static Task<IEnumerable<T>> FetchAllInternalAsync<T>(
            IEnumerable<T> objects, bool force, Task toAwait, CancellationToken cancellationToken) where T : ParseObject => toAwait.OnSuccess(_ =>
                                                                                                                                      {
                                                                                                                                          ...
                                                                                                                                          List<T> objectsToFetch = (from obj in objects
                                                                                                                                          ...
                                                                                                                                          Dictionary<string, Task<IEnumerable<ParseObject>>> findsByClass =
        ...
        /// </summary>
        /// <param name="cancellationToken">The cancellation token.</param>
        public Task DeleteAsync(CancellationToken cancellationToken) => taskQueue.Enqueue(toAwait => DeleteAsync(toAwait, cancellationToken),
              cancellationToken);
        ...
        public static Task DeleteAllAsync<T>(IEnumerable<T> objects) where T : ParseObject => DeleteAllAsync(objects, CancellationToken.None);
            ...
            IEnumerable<T> objects, CancellationToken cancellationToken) where T : ParseObject
        {
            ...
            return ParseObject.EnqueueForAll<object>(uniqueObjects, toAwait =>
            {
                ...
                return toAwait.OnSuccess(_ =>
                {
                    IList<Task> deleteTasks = ObjectController.DeleteAllAsync(states,
                ...
                }).Unwrap().OnSuccess(t =>
                {
                    ...
                    foreach (ParseObject obj in uniqueObjects)
                    {
                        obj.IsDirty = true;
                    }
        ...
        /// used parameters.
        /// </summary>
        private static void CollectDirtyChildren(object node, IList<ParseObject> dirtyChildren) => CollectDirtyChildren(node,
                dirtyChildren,
        ...
        /// </summary>
        private static bool CanBeSerializedAsValue(object value) => DeepTraversal(value, yieldRoot: true)
              .OfType<ParseObject>()
        ...
        protected virtual bool IsKeyMutable(string key) => true;
        ...
        public ParseACL ACL
        {
            get => GetProperty<ParseACL>(null, "ACL");
 ...
 bool IsNew
      ...
      set
        ...
        {
            IDictionary<string, string> mappings = SubclassingController.GetPropertyMappings(ClassName);
